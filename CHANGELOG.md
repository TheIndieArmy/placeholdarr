...
- **Fetch from TMDB**: Opt-in gate; off keeps Arr posters for faster sync and art refresh.
- **Language**: Fixed TMDB language for library and placeholder posters; exact match only.
- **Original language first**: Optional; original wins over Language, then Arr if neither is available.
...
- **Optional APIs**: Collection Sources renamed; TMDB also powers poster language.
...
- **Fetch from TMDB**: Opt-in Status Updates / Look and feel gate; off keeps Radarr/Sonarr posters for faster sync and art refresh.
- **Language**: When the gate is on, picks a fixed TMDB poster language; only an exact language match is stored, otherwise Arr stays the fallback so full sync can retry empties later.
- **Original language first**: Optional toggle (when the gate is on) tries each title's TMDB original language before Language, then Arr if neither is available.
...
- **Series add season posters**: Series add applies Sonarr season images and resolves poster language before materialization.
...
- **What's new (0.9.26)**: Startup ack notice for poster language (TMDB key under Optional APIs).

### Changed

- **Optional APIs**: Settings → Collection Sources is renamed; TMDB covers poster language as well as Collections lists, with Trakt and Tautulli grouped under the same page.
