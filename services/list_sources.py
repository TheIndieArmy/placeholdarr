...
def fetch_trakt_list(reference: str, media_type: str, limit: int = 200) -> list[dict[str, Any]]:
    ...
    if not client_id:
        raise ListSourceError(
            "Trakt Client ID is not configured (Settings → Optional APIs). Creating a Trakt API app currently requires Trakt VIP."
        )
...
def fetch_tautulli(
    ...
    *,
    ...
    if not base or not api_key:
        raise ListSourceError("Tautulli URL and API key are not configured (Settings → Optional APIs)")
