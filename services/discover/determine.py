"""Determination for TMDB Discover movie placeholders."""
...
def _overlay_flags_from_rows(rows: list[ArrMovieOverlay]) -> tuple[bool, bool]:
    ...
    # any-instance vs preferred-key currently both mean "any monitored overlay"
...
def _overlay_flags(session, tmdb_id: int) -> tuple[bool, bool]:
    """Return (any_has_file, monitored_skip)."""
    ...
    *,
    ...
    if monitored_skip:
        if row.has_placeholder:
            return DETERMINATION_OBSOLETE
        return DETERMINATION_NOT_NEEDED
    if row.has_placeholder and row.placeholder_filepath:
        return DETERMINATION_EXISTS
    return DETERMINATION_NEEDS


def list_undetermined_tmdb_ids(*, session=None) -> list[int]:
    """Catalog rows that have never been scored (needed after seed / interrupted sync)."""
    own = session is None
...
def run_discover_determination(*, session=None, tmdb_ids: list[int] | None = None) -> dict[str, Any]:
    ...
    try:
        q = session.query(TmdbMovie)
        if tmdb_ids is not None:
            ids = [int(x) for x in tmdb_ids]
            if not ids:
                return stats
            q = q.filter(TmdbMovie.tmdb_id.in_(ids))
        rows = q.all()
        overlay_q = session.query(ArrMovieOverlay)
        if tmdb_ids is not None:
            overlay_q = overlay_q.filter(ArrMovieOverlay.tmdb_id.in_([int(x) for x in tmdb_ids]))
        by_tmdb: dict[int, list[ArrMovieOverlay]] = defaultdict(list)
        for ov in overlay_q.all():
            by_tmdb[int(ov.tmdb_id)].append(ov)

        scope_note = f" (scoped to {len(rows)} id(s))" if tmdb_ids is not None else ""
        logger.info(
            f"Discover determination: scoring {len(rows)} catalog movie(s){scope_note} "
            f"({len(by_tmdb)} with Arr overlay)",
            extra={"emoji_type": "info"},
        )
        now = datetime.now(timezone.utc)
        for i, row in enumerate(rows, start=1):
            det = resolve_tmdb_movie_determination(
                row, overlay_rows=by_tmdb.get(int(row.tmdb_id), [])
            )
            ...
            if i % 2000 == 0:
                logger.info(
                    f"Discover determination: scored {i}/{len(rows)}…",
                    extra={"emoji_type": "info"},
                )
        session.commit()
        return stats
