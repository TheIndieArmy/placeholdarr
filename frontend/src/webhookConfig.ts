  ...
  services: [
    {
      ...
      notes: [
        "Placeholdarr currently supports up to 4 Radarr instances per deployment",
    ...
    },
    {
      ...
      notes: [
        "Placeholdarr currently supports up to 4 Sonarr instances per deployment",
        "Each instance must have its own webhook with the correct instance parameter",
        "The instance parameter uses each instance's stable webhook key shown in ARR settings",
      ],
    },
  ...
  services: [
    ...
    },
    {
      ...
      notes: [
        ...
        "Automations → New automation → “Start from scratch”",
        "When… choose “A stream is first seen”; leave Who this applies to on Everyone; leave And only if… empty",
        "Then do… add “Send Notification” and pick the Placeholdarr destination from step 1",
        "Destinations no longer pick stream events themselves; routing is via Automations",
        "Jellyfin/Emby: Tracearr SSE on the media server is required; without it, Tracearr polls and can miss short placeholder plays between polls",
        "Emby: Combined with Tracearr, Emby users hear playback without Emby Premiere (native Emby webhooks require Premiere)",
